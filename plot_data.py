...
if __name__ == "__main__":
    ...
    for num_sensitive in num_sensitive_list:

        # get the start time to calculate execution time
        start_time = time.time()

        print("Calculating band matrix")
        df_square, items, sensitive_items = compute_band_matrix(dataset=df, bm_size=bm_size,
    ...
    df_to_plot.to_csv(f"./Data_to_plot/BMS1_{bm_size}_{p_degree}_num_sensitive_change.csv", index=False)
